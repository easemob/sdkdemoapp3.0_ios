...
@property (nonatomic, copy) void (^selectedCompletion)(NSURL *url);
@end
...
- (void)viewWillLayoutSubviews
{
    ...
    tableFrame.origin.y += (60+4);
    tableFrame.size.height -= (28+4);
...
- (void)sharePath:(NSString*)path
{
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"发送" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [weakself btnCloseClick];
        weakself.selectedCompletion(url);
    }]];
...
- (void)showSandboxBrowser {
    if (_window == nil) {
        ...
        _window.frame = CGRectInset(keyFrame, 0, 0);
        ...
        __weak typeof(self) weakself = self;
        [_ctrl setSelectedCompletion:^(NSURL *url) {
            weakself.sendCompletion(url);
        }];
